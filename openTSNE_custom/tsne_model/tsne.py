...
from openTSNE_custom.tsne_model import _tsne
from openTSNE_custom.tsne_model import initialization as initialization_scheme
from openTSNE_custom.tsne_model.affinity import Affinities, MultiscaleMixture
from openTSNE_custom.tsne_model.quad_tree import QuadTree
from openTSNE_custom.tsne_model import utils
...
def _handle_nice_params(embedding: np.ndarray, optim_params: dict) -> None:
    ...
    if isinstance(negative_gradient_method, str) and negative_gradient_method == "auto":
        ...
        log.info(
            "Automatically determined negative gradient method `{negative_gradient_method}`"
        )
...
class TSNEEmbedding(np.ndarray):
    ...
    def transform(
        ...
        if "perplexity" in affinity_signature.parameters:
            ...
            raise TypeError(
                "`transform` currently does not support "
                "{self.affinities.__class__.__name__} affinities. Please use "
                "`prepare_partial` and `optimize` to add points to the embedding."
            )
    ...
    def prepare_partial(self, X, initialization="median", k=25, **affinity_params):
        ...
        **affinity_params: dict
            ...
            Please see individual :class:`~tsne_model.affinity.Affinities`
    ...
    def prepare_interpolation_grid(self, padding=0.25):
        ...
        padding: float
            ...
            transform. This parameter specifies how much empty space should be
...
class TSNE(BaseEstimator):
    ...
    def fit(self, X=None, affinities=None, initialization=None):
        ...
        affinities: Optional[tsne_model.affinity.Affinities]
    ...
    def prepare_initial(self, X=None, affinities=None, initialization=None):
        ...
        if affinities is None:
            ...
            if not isinstance(affinities, Affinities):
                raise ValueError(
                    "`affinities` must be an instance of `tsne_model.affinity.Affinities`"
                )
            log.info(
        ...
        elif initialization == "schroedinger":
            embedding = initialization_scheme.schroedinger(
                X,
                n_neighbors=self.se_neighbors,
                metric=self.metric,
                random_state=self.random_state,
                verbose=self.verbose
            )
