...
import openTSNE_custom.affinity
import openTSNE_custom.initialization
import numpy as np
from openTSNE_custom.callbacks import VerifyExaggerationError
...
class TestTSNECorrectness(unittest.TestCase):
    ...
    def test_advanced_flow(self):
        ...
        embedding = embedding.optimize(20)  # type: openTSNE_custom.TSNEEmbedding
        self.assertFalse(np.any(np.isnan(embedding)))
    ...
    def test_bh_transform_with_point_subsets_using_perplexity_nn(self):
        ...
        # Set up the initial embedding
        init = openTSNE_custom.initialization.pca(x_train)
        affinity = openTSNE_custom.affinity.PerplexityBasedNN(x_train, method="exact")
        embedding = openTSNE_custom.TSNEEmbedding(
