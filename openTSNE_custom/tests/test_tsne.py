...
import openTSNE_custom
from openTSNE_custom import affinity
from openTSNE_custom import initialization
from openTSNE_custom.affinity import PerplexityBasedNN
from openTSNE_custom.nearest_neighbors import NNDescent
from openTSNE_custom.tsne import kl_divergence_bh, kl_divergence_fft
from openTSNE_custom.utils import is_package_installed

np.random.seed(42)
affinity.log.setLevel(logging.ERROR)

TSNE = partial(openTSNE_custom.TSNE, neighbors="exact", negative_gradient_method="bh")
...
class TestTSNEParameterFlow(unittest.TestCase):
    ...
    @patch("tsne_model.tsne.gradient_descent.__call__")
...
class TSNEInitialization(unittest.TestCase):
    ...
    def test_same_unoptimized_initializations_for_transform(self):
        ...
        embedding = openTSNE_custom.TSNE(
...
class TestRandomState(unittest.TestCase):
    ...
    @patch("tsne_model.initialization.random", wraps=openTSNE_custom.initialization.random)
    @patch("tsne_model.nearest_neighbors.Sklearn", wraps=openTSNE_custom.nearest_neighbors.Sklearn)
    def test_random_state_parameter_is_propagated_random_init_exact(self, init, neighbors):
        random_state = 1

        tsne = openTSNE_custom.TSNE(
            neighbors="exact",
    ...
    @patch("tsne_model.initialization.pca", wraps=openTSNE_custom.initialization.pca)
    @patch("tsne_model.nearest_neighbors.Annoy", wraps=openTSNE_custom.nearest_neighbors.Annoy)
...
class TestDefaultParameterSettings(unittest.TestCase):
    def test_default_params_simple_vs_complex_flow(self):
        ...
        assert len(
            get_shared_parameters(openTSNE_custom.TSNE, openTSNE_custom.tsne.gradient_descent.__call__)
        ) > 0, \
        ...
        mismatching = get_mismatching_default_values(
            openTSNE_custom.TSNE,
            openTSNE_custom.tsne.gradient_descent.__call__,
        )
...
class TestGradientDescentOptimizer(unittest.TestCase):
    ...
    def test_pickling(self):
        obj = openTSNE_custom.tsne.gradient_descent()
        obj.gains = np.ones(5)
    ...
    def test_gains_is_always_numpy_array(self):
        ...
        self.assertNotIsInstance(embedding.optimizer.gains, openTSNE_custom.TSNEEmbedding)
...
class TestAffinityIntegration(unittest.TestCase):
    ...
    def test_transform_with_standard_affinity(self):
        init = openTSNE_custom.initialization.random(self.x)
        aff = openTSNE_custom.affinity.PerplexityBasedNN(self.x, 5, method="exact")
        embedding = openTSNE_custom.TSNEEmbedding(init, aff, negative_gradient_method="bh")
    ...
    def test_transform_with_multiscale_affinity(self):
        init = openTSNE_custom.initialization.random(self.x)
        aff = openTSNE_custom.affinity.Multiscale(self.x, [2, 5], method="exact")
        embedding = openTSNE_custom.TSNEEmbedding(init, aff, negative_gradient_method="bh")
        embedding.optimize(100, inplace=True)
    ...
    def test_transform_with_nonstandard_affinity(self):
        ...
        with `transform`."""
        init = openTSNE_custom.initialization.random(self.x)
        aff = openTSNE_custom.affinity.Uniform(self.x, 5, method="exact")
        embedding = openTSNE_custom.TSNEEmbedding(init, aff, negative_gradient_method="bh")
        embedding.optimize(100, inplace=True)
...
class TestTSNEEmebedding(unittest.TestCase):
    def test_pickling(self):
        ...
        self.assertIsInstance(loaded_obj, openTSNE_custom.TSNEEmbedding)
        self.assertIsInstance(loaded_obj.affinities, openTSNE_custom.affinity.Affinities)
        self.assertEqual(4, loaded_obj.random_state)

    def test_pickling_with_transform(self):
        tsne = TSNE(random_state=4)
        embedding: openTSNE_custom.TSNEEmbedding = tsne.fit(np.random.randn(100, 4))
        loaded_obj: openTSNE_custom.TSNEEmbedding = pickle.loads(pickle.dumps(embedding))
...
class TestMisc(unittest.TestCase):
    ...
    def test_interpolation_grid_not_called_using_bh(self):
        ...
        with patch("tsne_model.TSNEEmbedding.prepare_interpolation_grid") as prep_grid:
            tsne = openTSNE_custom.TSNE(negative_gradient_method="bh")
