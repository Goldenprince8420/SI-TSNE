...
import openTSNE_custom.callbacks
...
class openTSNENNDescent(TSNEBenchmark):
    def run(self, n_samples=1000, random_state=None):
        ...
        start = time.time()
        start_aff = time.time()
        affinity = openTSNE_custom.affinity.PerplexityBasedNN(
            x,
        ...
        print("tsne_model: NN search", time.time() - start_aff, flush=True)

        init = openTSNE_custom.initialization.random(
        ...
        print("tsne_model: Optimization", time.time() - start_optim)
        print("tsne_model: Full", time.time() - start, flush=True)
...
class FItSNE(TSNEBenchmark):
    def run(self, n_samples=1000, random_state=-1):
        ...
        if random_state == -1:
            init = openTSNE_custom.initialization.random(x, n_components=2)
        else:
            init = openTSNE_custom.initialization.random(
