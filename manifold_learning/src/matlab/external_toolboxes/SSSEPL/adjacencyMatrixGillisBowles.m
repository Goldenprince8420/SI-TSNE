function A = adjacencyMatrixGillisBowles(varargin)
% adjacencyMatrixGillisBowles - construct adjacency matrix from hyperspectral 
%   image using Gillis-Bowles algorithm
% usage: A = adjacencyMatrixGillisBowles(imageData,rad,sigmaF,sigmaP);
%
% arguments:
...
for j = 1:numNeighbors
    ...
    specWeight = exp(-acos(sum(imageDataVec.*imageDataPadVec(newInd,:),2)./(sqrt(sum(imageDataVec.^2,2)).*sqrt(sum(imageDataPadVec(newInd,:).^2,2))))/sigmaF2);
