...
from manifold_learning.src.python.utils.eigenvalue_decomposition import EigSolver
from manifold_learning.src.python.utils.graph import create_laplacian, maximum, \
    compute_adjacency
from manifold_learning.src.python.utils.nearestneighbor_solver import knn_scikit
    ...
    * Schroedinger Eigenmaps w/ Nondiagonal Potentials for Spatial-Spectral
    ...
    def _potential(self, X, y=None):
        ...
        # create the similarity potential matrix
            ...
            raise ValueError('Sorry. This method is unavailable at the moment.')
...
def graph_embedding(adjacency, data,
    ...
    if ss_potential is not None:  # spatial-spectral potential
        if not alpha:
            alpha = 17.78
        alpha = get_alpha(alpha, L, ss_potential)
        A = L + alpha * ss_potential

    elif pl_potential is not None:  # partial-labels potential
...
def sim_potential(X, potential='sim',
    ...
    Wang - Heterogeneous Domain Adaption using Manifold Alignment
        https://goo.gl/QaLTA4
