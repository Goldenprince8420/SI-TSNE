...
from manifold_learning.src.python.utils.knn_solvers import KnnSolver
...
def create_laplacian(Adjacency,
    ...
    * Diagonal        - an NxN diagonal array
    """
    L, D = laplacian(Adjacency, normed=norm_lap,
                     return_diag=True)
    D = spdiags(data=D,
                diags=[0],
                m=Adjacency.shape[0],
...
def create_feature_mat(X, A, sparse=None):
    ...
    A : (nxn) array
        ...
        constructed as a Laplacian matrix, adjacency matrix or diagonal
        degree matrix.
...
def laplacian_test():
    ...
    tol = 1E-1
    print('Different between the Laplacian Matrix values close with tol: {tol}?'.format(tol=tol))
